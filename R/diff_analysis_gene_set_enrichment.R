...
differential_analysis_scExp = function(scExp, de_type = "one_vs_rest",
  ...
  if(de_type == "one_vs_rest"){ # compare each cluster to all the rest
    ...
    for(i in 1:(as.integer(nclust)-1)){
      mygps = list(affectation[which(affectation$chromatin_group==paste0("C", i)), "cell_id"])
      names(mygps) = paste0('C', i)
      groups = names(mygps)
      for(j in (i+1):as.integer(nclust)){
        myrefs = list(affectation[which(affectation$chromatin_group==paste0("C", j)), "cell_id"])
        names(myrefs) = paste0('C', j)
        refs = names(myrefs)
        tmp_result = geco.CompareWilcox(dataMat = counts, annot = affectation, ref = myrefs, groups = mygps, featureTab = feature)
...
gene_set_enrichment_analysis_scExp = function(scExp, enrichment_qval = 0.1, ref = "hg38",
                                              ...
                                              use_peaks = F)
{
  stopifnot(is(scExp, "SingleCellExperiment"), is.character(ref), is.numeric(enrichment_qval), is.numeric(peak_distance),
          is.numeric(qval.th), is.numeric(cdiff.th))
  ...
  if(is.null(use_peaks)) use_peaks = F
  ...
  if(is.null(GenePool)) {
    message(paste0("ChromSCape::gene_set_enrichment_analysis_scExp - Selecting ",ref," genes from Gencode."))
  ...
  for(i in 1:length(groups)){
    gp = groups[i]
    ref = diff$refs[i]
    signific = res$ID[which(res[, paste("qval", gp, sep=".")] <= qval.th & abs(res[, paste("cdiff", gp, sep=".")]) > cdiff.th)]
    significG = unique(annotFeat_long$Gene[annotFeat_long$distance < peak_distance & annotFeat_long$ID %in% signific])
    over = res$ID[which(res[, paste("qval", gp, sep=".")] <= qval.th & res[, paste("cdiff", gp, sep=".")] > cdiff.th)]
    overG = unique(annotFeat_long$Gene[annotFeat_long$distance < peak_distance & annotFeat_long$ID %in% over])
    under = res$ID[which(res[, paste("qval", gp, sep=".")] <= qval.th & res[, paste("cdiff", gp, sep=".")] < -cdiff.th)]
    underG = unique(annotFeat_long$Gene[annotFeat_long$distance < peak_distance & annotFeat_long$ID %in% under])
    if( !is.null(use_peaks)){
      if(use_peaks ==TRUE){
        refined_annotation = scExp@metadata$refined_annotation
        signific_associated_peak = refined_annotation$peak_ID[refined_annotation$window_ID %in% signific]
        over_associated_peak = refined_annotation$peak_ID[refined_annotation$window_ID %in% over]
        under_associated_peak = refined_annotation$peak_ID[refined_annotation$window_ID %in% under]
        signific_associated_gene = refined_annotation$Gene[refined_annotation$peak_ID %in% signific_associated_peak & refined_annotation$distance < peak_distance]
        over_associated_gene = refined_annotation$Gene[refined_annotation$peak_ID %in% over_associated_peak & refined_annotation$distance < peak_distance]
        under_associated_gene = refined_annotation$Gene[refined_annotation$peak_ID %in% under_associated_peak & refined_annotation$distance < peak_distance]
    ...
    if(length(significG)){
      enrich.test = geco.enrichmentTest(gene.sets = GeneSets, mylist = significG, possibleIds = GenePool)
      enrich.test = data.frame(Gene_set_name = rownames(enrich.test), enrich.test, check.names = FALSE)
      enrich.test = merge(subset(GeneSetsDf, select=-Genes), enrich.test, by.x="Gene.Set", by.y="Gene_set_name", all.y = TRUE, sort = FALSE ) ## Get class of gene set
    ...
    if(length(overG)){
      enrich.test = geco.enrichmentTest(gene.sets = GeneSets, mylist = overG, possibleIds = GenePool)
      enrich.test = data.frame(Gene_set_name = rownames(enrich.test), enrich.test, check.names = FALSE)
      enrich.test = merge( subset(GeneSetsDf, select=-Genes), enrich.test, by.x="Gene.Set", by.y="Gene_set_name", all.y = TRUE, sort = FALSE ) ## Get class of gene set
      enrich.test = enrich.test[order(enrich.test$`p-value`),]
      ind = which(enrich.test$`q-value`<= enrichment_qval)
    ...
    if(length(underG)){
      enrich.test = geco.enrichmentTest(gene.sets = GeneSets, mylist = underG, possibleIds = GenePool)
      enrich.test = data.frame(Gene_set_name = rownames(enrich.test), enrich.test, check.names = FALSE)
      enrich.test = merge( subset(GeneSetsDf, select=-Genes), enrich.test, by.x="Gene.Set", by.y="Gene_set_name", all.y = TRUE, sort = FALSE ) ## Get class of gene set
      enrich.test = enrich.test[order(enrich.test$`p-value`),]
      ind = which(enrich.test$`q-value`<= enrichment_qval)
