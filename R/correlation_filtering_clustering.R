...
#'
#' @examples 
correlation_and_hierarchical_clust_scExp <- function(scExp, correlation = "pearson", hc_linkage = "ward.D")
  {
  ...
  pca_t <- Matrix::t(pca)
  cor_mat = cor(pca_t, method = correlation)
  
  hc_cor = hclust(as.dist(1 - cor_mat), method = hc_linkage) 
  hc_cor$labels = rep("",length(hc_cor$labels))
  
  scExp@metadata$hc_cor = hc_cor
  reducedDim(scExp,"Cor") <- as.matrix(cor_mat)
...
filter_correlated_cell_scExp <- function(scExp, random_iter = 50, corr_threshold = 99,
  ...
  tab = as.data.frame(reducedDim(scExp,"Cor"))[,selection_cor_filtered]
  reducedDim(scExp,"Cor") = tab

  set.seed(seed)
  tsne = Rtsne(reducedDim(scExp,"PCA"), dims = 2, max_iter = 1000, pca = FALSE, theta = 0,
               perplexity = choose_perplexity(reducedDim(scExp,"PCA")), verbose = verbose)
  reducedDim(scExp,"TSNE") = as.data.frame(tsne$Y)
...
consensus_clustering_scExp <- function(scExp, prefix = NULL, maxK = 10, reps = 100, pItem = 0.8, pFeature=1, distance = "pearson",
  ...
  consclust <- ConsensusClusterPlus(pca_t, maxK = maxK, reps = reps, pItem = pItem, pFeature = pFeature,
                                    title = prefix,
                                    clusterAlg = clusterAlg, distance = distance,
                                    innerLinkage = innerLinkage, finalLinkage = finalLinkage, seed = seed, plot = plot_consclust)
  
  
  icl <- calcICL(consclust, plot = plot_icl, title = prefix)
...
choose_cluster_scExp <- function(scExp, nclust = 3, hc_linkage = "ward.D"){
  ...
  if( ! "icl" %in% names(scExp@metadata)) stop(
    ...
    hcc <- hclust(distPearson(t(mat.cc)), method=hc_linkage)
    
    scExp = colors_scExp(scExp = scExp, annotCol = "chromatin_group", color_by = "chromatin_group", color_df = NULL)
  
    reducedDim(scExp,"ConsensusAssociation") =  scExp@metadata$consclust[[nclust]]$ml
    scExp@metadata$hc_consensus_association = hclust(as.dist(1 - reducedDim(scExp,"ConsensusAssociation")), method="ward.D")
...
num_cell_in_cluster_scExp <- function(scExp){
  ...
  table_raw <- as.data.frame.matrix(table(as.data.frame(colData(scExp))[,c("chromatin_group","sample_id")]))
  
  #Overall goodness of fit testing : how fairly are cells  allocated between the clusters ?
  chi <- chisq.test(x = as.matrix(table_raw), correct = FALSE)
  ...
  tab <- table_raw
  if(length(unique(colData(scExp)$sample_id))==1){
  ...
  colors_chromatin_group = col2hex(unique(colData(scExp)[,"chromatin_group_color"]))
  colors_sample_id = col2hex(unique(colData(scExp)[,"sample_id_color"]))
  ...
  samples = cell_spec(colnames(tab)[2:(length(colors_sample_id)+1)], color="white", bold = T, background = colors_sample_id)
  tab[1,2:(length(colors_sample_id)+1)] = samples
  colnames(tab)[2:(length(colors_sample_id)+1)] = rep("",length(colors_sample_id))
  
  tab[2:(length(colors_chromatin_group)+1),"Cluster"] = cell_spec(tab[2:(length(colors_chromatin_group)+1),"Cluster"],
                                                                  color="white", bold = T, background = colors_chromatin_group)
  
  tab %>% kable(escape = F, align="c") %>%
    kable_styling(c("striped", "condensed"), full_width = F) %>%
    group_rows("Total", dim(tab)[1], dim(tab)[1])
}
